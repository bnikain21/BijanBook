...
import { getAllCategories, getTransactionsByMonth, insertTransaction, Category } from "../db/queries";
...
export default function AddScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      getTransactionsByMonth(month).then((txs) => {
        const counts: Record<number, number> = {};
        for (const tx of txs) counts[tx.categoryId] = (counts[tx.categoryId] ?? 0) + 1;
        setTxCounts(counts);
      });
    }, [month])
  );
  ...
  const sortedCategories = useMemo(() =>
    [...categories].sort((a, b) => {
      const diff = (txCounts[b.id] ?? 0) - (txCounts[a.id] ?? 0);
  ...
  useEffect(() => {
    if (sortedCategories.length > 0 && categoryId === null) {
  ...
  async function handleSubmit() {
    ...
    setAmount("");
    setNotes("");
    if (sortedCategories.length > 0) setCategoryId(sortedCategories[0].id);
...
const styles = StyleSheet.create({
  ...
  fieldRowFocused: {
    // Icon color handled inline; no extra bg needed
  },
  fieldIconWrap: {
    width: 28,
    alignItems: "center",
